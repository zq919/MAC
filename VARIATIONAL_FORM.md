...
+{\color{red}{\left(-\langle \bar p_h,\bar{\boldsymbol{v}}_h\cdot\boldsymbol{n}\rangle_{\Gamma_N}\right)}} \\
...
+{\color{red}{\left(-\langle \bar{\boldsymbol{u}}_h\cdot\boldsymbol{n},\bar q_h\rangle_{\Gamma_N}\right)}}
+\epsilon_p(p_h,q_h)_\Omega \\
&= (\boldsymbol{f},\boldsymbol{v}_h)_\Omega
+{\color{red}{\left(-\langle \boldsymbol{g}_N,\bar{\boldsymbol{v}}_h\rangle_{\Gamma_N}\right)}}.
\end{aligned}
$$

为兼容不同 Markdown/LaTeX 渲染器，上述主方程再给出一个 `math` fenced 版本（部分平台仅识别这种语法）：

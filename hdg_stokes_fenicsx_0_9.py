...
This script follows the HDG structure used by the convection-diffusion example,
but solves the steady Stokes problem with interior and trace unknowns for both
velocity and pressure on the unit square.
...
def velocity_exact(x):
    """Exact velocity from the provided Stokes benchmark."""
    module = ufl if isinstance(x, ufl.SpatialCoordinate) else np
    return ufl.as_vector(
        (
            -x[0] ** 2 * (x[0] - 1.0) ** 2 * x[1] * (x[1] - 1.0) * (2.0 * x[1] - 1.0),
            x[0] * (x[0] - 1.0) * (2.0 * x[0] - 1.0) * x[1] ** 2 * (x[1] - 1.0) ** 2,
        )
    ) if module is ufl else np.vstack(
        (
            -x[0] ** 2 * (x[0] - 1.0) ** 2 * x[1] * (x[1] - 1.0) * (2.0 * x[1] - 1.0),
            x[0] * (x[0] - 1.0) * (2.0 * x[0] - 1.0) * x[1] ** 2 * (x[1] - 1.0) ** 2,
        )
...
def pressure_exact(x):
    """Exact pressure from the provided Stokes benchmark."""
    module = ufl if isinstance(x, ufl.SpatialCoordinate) else np
    return x[0] ** 6 - x[1] ** 6 if module is ufl else x[0] ** 6 - x[1] ** 6
...
# HDG spaces: interior velocity/pressure and facet traces.
gdim = msh.geometry.dim
V = fem.functionspace(msh, ("Discontinuous Lagrange", k, (gdim,)))
Vbar = fem.functionspace(facet_mesh, ("Discontinuous Lagrange", k, (gdim,)))
...
# Diffusion-like HDG bilinear form for velocity

a = (
    nu * inner(grad(u_h), grad(v_h)) * dx_c
    - nu * inner(u_h - ubar_h, dot(grad(v_h), n)) * ds_c(cell_boundaries)
    - nu * inner(dot(grad(u_h), n), v_h - vbar_h) * ds_c(cell_boundaries)
    + nu * (alpha / h) * inner(u_h - ubar_h, v_h - vbar_h) * ds_c(cell_boundaries)
)

# Pressure-velocity coupling from the provided HDG Stokes formulation.
b_vp = -inner(p_h, div(v_h)) * dx_c + inner(dot(v_h, n), pbar_h) * ds_c(cell_boundaries)
b_uq = -inner(q_h, div(u_h)) * dx_c + inner(dot(u_h, n), qbar_h) * ds_c(cell_boundaries)
...
# Dirichlet condition on the trace velocity: \\bar{u}_h = u_exact on boundary.
...
# Build nullspace corresponding to constant pressure / trace-pressure pair.
...
par_print(comm, f"||u - u_h||_L2 = {e_u}")
par_print(comm, f"||p - p_h||_L2 = {e_p}")
par_print(comm, f"||div(u_h)||_L2 = {e_div}")
